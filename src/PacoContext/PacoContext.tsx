...
const mapDispatchToProps = (dispatch) => ({
  ...
  setPacoRefProp: (ref) => dispatch(setPacoRef(ref)),
});
...
export const PacoContext = connector(({ dataset, pacoAttributes, pacoConstraints, setPacoRefProp, setCurrentAggregation, currentAggregation }: Props) => {
  ...
  React.useEffect(() => {
    setPacoRefProp(pacoRef?.current);
    }, [pacoRef])
          ...
          if (Object.keys(data[0]).filter((item) => item.includes('constraintrange')).length > 0) {
            ...
            const filteredVectors = dataset.vectors.filter((row) => {
              ...
              for (const i in constraintDims) {
                if ({}.hasOwnProperty.call(constraintDims, i)) {
                  ...
                  for (const j in constraintarray) {
                    if ({}.hasOwnProperty.call(constraintarray, j)) {
