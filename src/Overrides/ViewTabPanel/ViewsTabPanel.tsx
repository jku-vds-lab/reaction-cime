...
import { ComponentConfig, DetailViewActions } from 'projection-space-explorer';
import { AppState } from '../../State/Store';

type DetailViewChooserProps = {
  overrideComponents: ComponentConfig;
  splitRef: React.LegacyRef<any>;
};

function instantiateElement(view: JSX.Element | (() => JSX.Element) | ConnectedComponent<any, any>, splitRef: React.LegacyRef<any>) {
  if (!view) return null;
...
export function ViewsTabPanel({ overrideComponents, splitRef }: DetailViewChooserProps) {
  ...
  const detailView = useSelector((state: AppState) => state.detailView);
