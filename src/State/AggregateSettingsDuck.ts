...
const initialColorMapSettings = { aggregateColor: initialAggregateColor, scale_obj: null, colorscale: D3_CONTINUOUS_COLOR_SCALE_LIST[0], useVSUP: true, valueFilter: []} as ColorMapSettingsType
...
const initialAdvancedSettings = { deriveRange: true, valueRange: null, uncertaintyRange: null, 
    variableIndex: {valueVariableIndex: 0, uncertaintyVariableIndex: 0}, 
    aggregationMethod: {valueAggregationMethod: AggregationMethod.MAX, uncertaintyAggregationMethod: AggregationMethod.MIN}, sampleSize: 20 } as AdvancedSettingsType
...
const initialState = { colormapSettings: initialColorMapSettings, stepperSettings: initialStepperSettings, advancedSettings: initialAdvancedSettings, selectAttribute: null, selectAttributeInfo: null } as AggregateSettingsState
...
const aggregateSettingsSlice = createSlice({
    ...
    reducers:{
        ...
        removeValueFilter(state, action: PayloadAction<string>){
            var index = state.colormapSettings.valueFilter.indexOf(action.payload);
            if (index !== -1) {
...
export const { setCurStep, setSelectAttribute, setAggregateColor, setAggregateColorMapScale, setValueRange, setUncertaintyRange, 
    toggleDeriveRange, setDeriveRange, setAggregateColorScale, toggleUseVSUP, addValueFilter, 
